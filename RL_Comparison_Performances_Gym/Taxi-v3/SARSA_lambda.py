...
parser = argparse.ArgumentParser(description='Hyperparameter for SARSA Lambda agent')
...
class SARSAlambda_Agent():
    ...
    def learn(self, state, action, reward, state_, action_):
        ...
        target = reward + (self.discount_factor*self.Q[(state_, action_)])
        predict = self.Q[(state, action)]
        diff_error = target - predict
        
        for act in range(self.n_actions):            
            self.eligibility_trace[(state,act)] = 0
...
def plot_result(r, avg_r):
    ...
    plt.title('Scores obtained by SARSA Lambda agent across the games')
    plt.ylabel('Score')
    plt.xlabel('Games')
...
if __name__ == '__main__':
    ...
    agent = SARSAlambda_Agent(alpha=args.lr, gamma=args.gamma, n_actions=env.nA, n_states=env.nS, eps_start=args.eps, eps_min=args.eps_min, eps_dec=args.eps_decay)
    scores = []
    avg_scores_list = []
